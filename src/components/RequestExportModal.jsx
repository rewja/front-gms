...
const RequestExportModal = ({
  ...
  // Update custom range when requests change
  useEffect(() => {
    if (requests.length > 0) {
      setCustomRange({ from: 1, to: Math.min(requests.length, 10) });
    }
  }, [requests]);
  ...
  const getExportData = () => {
    ...
    switch (exportScope) {
      case "page": {
        ...
        dataToExport = requests.slice(startIndex, endIndex);
        break;
      }
      case "selected": {
        dataToExport = requests.filter((request) =>
      ...
      case "custom": {
        ...
        const toIndex = Math.min(requests.length, customRange.to);
        dataToExport = requests.slice(fromIndex, toIndex);
        break;
      }
      default: {
  ...
  const formatRequestData = (request, index) => {
    return {
      "No": index + 1,
      "ID": request.id,
      ...
      "Amount": request.amount ? `Rp ${Number(request.amount).toLocaleString()}` : "-",
      "Status": request.status,
      "Requested By": request.user?.name || "-",
      "Department": request.user?.department || "-",
      "Request Date": safeFormatDate(request.created_at),
      "Approved Date": safeFormatDate(request.approved_at),
      "GA Note": request.ga_note || "-",
    };
  };
  ...
  const exportToPDF = async () => {
    try {
      ...
      // Header
      pdf.setFontSize(16);
      pdf.text("Request Management Report", pageWidth / 2, 20, { align: "center" });

      pdf.setFontSize(10);
      pdf.text(`Export Date: ${safeFormatDate(new Date())}`, 14, 30);
      pdf.text(`Exported by: ${user?.name}`, 14, 35);
      pdf.text(`Total Requests: ${dataToExport.length}`, 14, 40);

      const startY = 50;

      // Table data
      const tableData = dataToExport.map((request, index) => {
        const formatted = formatRequestData(request, index);
        return [
          formatted["No"],
          formatted["ID"],
          formatted["Item Name"],
      ...
      const fileName = `request_management_${format(
        new Date(),
        "yyyyMMdd_HHmm"
      )}.pdf`;
      pdf.save(fileName);
  ...
  const exportToExcel = async () => {
    try {
      ...
      const excelData = dataToExport.map((request, index) =>
        formatRequestData(request, index)
      );
      ...
      const metaData = [
        ...
        { Field: "Total Requests", Value: dataToExport.length },
  ...
  const canExportSelected = selectedRequests.length > 0;
      ...
      <div className="bg-white rounded-xl shadow-xl border border-gray-200 max-w-md w-full max-h-[90vh] overflow-y-auto m-4 sm:m-6">
        <div className="flex items-center justify-between px-4 py-3 sm:px-6 sm:py-4 border-b border-gray-200">
          <h3 className="text-base sm:text-lg font-semibold text-gray-900 text-center w-full">
            {t("requests.exportRequests", { defaultValue: "Export Requests" })}
          </h3>
          <button
            onClick={onClose}
          ...
          <div>
            ...
            <div className="space-y-2">
              ...
              >
                ...
                />
                <span className="text-sm">
                  {t("common.selectedItems", { defaultValue: "Selected Items" })} ({selectedRequests.length})
                </span>
              </label>
              <label className="flex items-center">
                <input
              ...
              <label className="flex items-center">
                <input
                  ...
                  className="mr-3"
                />
                <span className="text-sm">{t("common.allItems", { defaultValue: "All Items" })} ({totalRequests})</span>
              </label>
            </div>
          </div>
              ...
              <div>
                ...
                <input
                  ...
                  onChange={(e) =>
                    setCustomRange({
                      ...customRange,
                      to: Math.min(totalRequests, parseInt(e.target.value) || totalRequests),
                    })
                  }
                  className="w-full px-3 py-2 border border-gray-300 rounded-md text-sm focus:outline-none focus:ring-2 focus:ring-accent-500 focus:border-accent-500"
                />
...
export default RequestExportModal;
